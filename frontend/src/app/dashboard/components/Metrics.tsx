        ...
        {metrics.map((item) => (
          <div key={item.name} className="border rounded-xl shadow-sm">
            <div className="aspect-[4/2] w-full flex flex-col p-2 text-center items-start mb-2">
              <span className="text-sm p-2 font-medium text-muted-foreground mb-2">{item.name}</span>
              ...
              <div className="flex justify-between flex-row items-center w-full mt-4 p-2">
                <span className="text-sm text-muted-foreground flex items-center gap-2">
                  {item.description}
                  {item.isPositive === true ? <TrendingUp className="w-4 h-4"/> : <TrendingDown className="w-4 h-4"/>}
                </span>
                <span className={`text-sm border-1 border-b-2 flex items-center font-semibold
                  ${item.isPositive === true ? 'text-green-500' : 'text-red-500'}`}>
                  {item.change} %
                  {item.isPositive === true ? <ArrowUp className="w-4 h-4" /> : <ArrowDown className="w-4 h-4" />}
                </span>
              </div>
            </div>
          </div>
